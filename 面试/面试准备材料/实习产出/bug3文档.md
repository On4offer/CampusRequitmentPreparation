...
1. **事务与一致性**
   - 入库单与明细、总额等强一致操作统一由 `@Transactional` 包裹的 Service 方法完成，Controller 仅编排，避免「先落主表再补明细」的中间态

2. **幂等与防重**
   - 对重复提交、网络重试场景可考虑业务幂等键或前端防抖

3. **横向排查**
   - 总部入库、其他入库入口是否同样存在「总额与明细不一致」风险，按同一清单扫一遍

4. **测试**
   - 覆盖：明细全空、部分失败、异常回滚后主表是否无脏数据
