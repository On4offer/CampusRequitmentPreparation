...
> 在**本次 Mapper 条件修复**已落地的前提下，从预防复发与长期维护角度的改进方向。

1. **根因与规范**
   - 将「排除渠道」与「是否移动端」等业务语义沉淀为注释或枚举常量，避免同类子查询复制粘贴时漏条件
   - 复杂报表 SQL 建议 Code Review 清单：移动端/网络订单等特殊维度是否一致

2. **测试**
   - 为关键报表补充回归 SQL 或集成测试用例（含 `isExclusionChannels=1` + 移动端数据场景）
   - 上线前用固定门店/固定数据集做基线对比

3. **可观测性**
   - 报表异常或数据波动时可增加数据校验任务（定时比对抽样）

4. **技术债**
   - 多处 Mapper 重复片段可考虑抽取 `<sql>` 片段或视图，降低分叉风险（需评估性能）
