...
> 本文件为 Bug1/2/3 修改汇总；分项细节见各独立文档。以下为跨 Bug 的共性改进方向。

1. **报表类 SQL（Bug1）**：子查询条件复杂时，用注释 + 回归用例 + 可选 SQL 片段抽取，降低漏改概率。
2. **业务校验（Bug2）**：校验规则收口、双路径（增/改）一致。
3. **事务与入库（Bug3）**：主从表 + 金额写入放在同一事务边界，并做同类接口巡检。
