...
1. **与上文「五、开发建议」的关系**：五偏日常规范，本节偏架构与长期演进，面试可答「若重做会怎么优化」。
2. **依赖与构建**：锁定 `package.json` 版本；大版本升级前在分支验证。
3. **导出等重功能**：大表导出考虑后端异步导出 + 下载链接，避免浏览器卡顿。
4. **组件沉淀**：业务表格、表单项抽成团队内组件库，减少复制粘贴。
