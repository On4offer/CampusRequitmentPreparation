...
**ORM（Object-Relational Mapping）** 的核心思想是：**用面向对象的方式操作关系型数据库**。
...
**MyBatis 的定位**：介于 JDBC 与 Hibernate 之间。SQL 由开发者编写，但参数绑定、结果映射、连接管理由框架完成。既保留 SQL 的灵活与可控，又消除 JDBC 的重复与繁琐。

> 💡 **一句话**：MyBatis 是“你写 SQL，我帮你做映射”的轻量 ORM 框架。
...
> 本章深入 MyBatis 内部架构，理解各组件职责与协作关系，为后续配置、映射、缓存、插件等打下基础。
